...
**对比表（材料用）：** `submission/tables/hydro_l0_vs_baseline_val.{md,csv}`、`hydro_l0_vs_baseline_test.{md,csv}`（由 `compare` 的 `--out-table-md` / CSV 选项生成，见 `docs/开发规划/下一步执行清单_云端评估前端与L0优化.md` §1.5）。
...
- 清单全文：`docs/开发规划/后续开发工作清单_未完成项与云端L0专项.md`（§2、§3）
- 命令细则：`docs/开发规划/下一步执行清单_云端评估前端与L0优化.md`
- 云端目录总览：`docs/工程手册/云端训练与目录归档.md`
