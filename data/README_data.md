...
| `eddy/` | 涡旋等 | 运行 ``python -m src.preprocess.eddy_dataset --export-yolo ...`` 后生成 `images/`、`labels/`、`dataset.yaml`。**Windows + 仓库路径含中文时**：读 NetCDF 需 ``h5netcdf``+``h5py``（见 `requirements.txt` 与 `src/utils/xarray_nc_open.py`）；写 PNG 已用 **PIL** 避免 ``cv2.imwrite`` 失败。 |
...
| `processed/anomaly/typhoon_kb/` | 台风查询事件索引与检索产物 | 运行 `scripts/build_typhoon_kb.py` 后生成 `events.json/csv`、`retrieval_index.json` |
...
**参赛方正式说明**：请优先阅读 **`服创数据集/数据集说明.md`**；仓库内摘要见 **`docs/架构与方法/命题方数据集说明.md`**（含海区、划分、指标与输入输出约定）。
...
- **缺测**：`short` 型常带 `_FillValue`（如 −30000），陆地格点解码后多为 **NaN**；预处理会统计并替换非有限值，Z-score 对 nan 做稳健估计，详见 `docs/过程与局限/实施过程与局限性.md`。
